...
from dataset import LongitudinalDatasetValidation, LongitudinalDataset
...
class LongitudinalDataModule(pl.LightningDataModule):
    ...
    def _get_subjects(self):
        ...
        reference_date_t0 = datetime.strptime(subjects[self.t0]['age'], self.date_format) if self.date_format else int(
            subjects[self.t0]['age'])
        reference_date_t1 = datetime.strptime(subjects[self.t1]['age'], self.date_format) if self.date_format else int(
            subjects[self.t1]['age'])
        for i in range(len(subjects)):
            age = datetime.strptime(subjects[i]['age'], self.date_format) if self.date_format else int(
                subjects[i]['age'])
            subjects[i]['age'] = (age - reference_date_t0) / (reference_date_t1 - reference_date_t0)
    ...
    def train_dataloader(self) -> tio.SubjectsLoader:
        transform = transforms.Compose([
            ...
            tio.RescaleIntensity(percentiles=(0.1, 99.9), include=['image']),
