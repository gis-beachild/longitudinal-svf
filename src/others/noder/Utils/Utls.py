...
#加载nii图像，同时完成预处理工作
def load_nii_int(imgPath,padding=(144,176,144)):
    img = tio.ScalarImage(imgPath)
    img = tio.transforms.CropOrPad(target_shape=padding)(img)
    img = tio.RescaleIntensity(percentiles=(0.1, 99.9))(img)
    img = tio.transforms.Clamp(out_min=0, out_max=1)(img)
    img_data = img[tio.DATA].squeeze(0).numpy()
    return img_data

def load_nii_seg(imgPath,padding=(144,176,144), num_classes= 20):
    img = tio.LabelMap(imgPath)
    img = tio.transforms.CropOrPad(target_shape=padding)(img)
    img = tio.RescaleIntensity(percentiles=(0.1, 99.9))(img)
    img = tio.transforms.Clamp(out_min=0, out_max=1)(img)
    img = tio.OneHot(num_classes=num_classes)(img)
    img_data = img[tio.DATA].numpy()
    return img_data

def load_nii_seg_scalar(imgPath, padding=(144,176,144)):
    img = tio.ScalarImage(imgPath)
    img = tio.transforms.CropOrPad(target_shape=padding)(img)
    img_data = img[tio.DATA].numpy()
    return img_data
...
def load_imgs_and_time(subject_path, padding=(128,128,128), num_classes=20, date_format=None):
    ...
    for t in lst_data:
        ...
        img_list.append(load_nii_int(imgPath=t[1], padding=padding))
        seg_list.append(load_nii_seg_scalar(imgPath=t[2], padding=padding))
    # 计算时间
    if date_format is not None:
        time_list = [cal_time(t) for t in time_list]
    start_time = time_list[0]
    last_time = time_list[-1] - start_time
    time_list = [(t - start_time) / last_time for t in time_list]
    return img_list, seg_list, time_list
