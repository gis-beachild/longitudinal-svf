...
from src.others.sgdir.utils import NCCLoss
...
class FlowNet3D(nn.Module):
    ...
    def get_u(self, I: torch.Tensor, J: torch.Tensor, xyz: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        Args:
            I (torch.Tensor): fixed image with size [B, 1, D, H, W]
            J (torch.Tensor): moving image with size [B, 1, D, H, W]
    ...
    def loss_flow(self, I: torch.Tensor, J: torch.Tensor, I_seg: torch.Tensor, J_seg: torch.Tensor, xyz: torch.Tensor, res: float) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Args:
            I (torch.Tensor): fixed image with size [B, 1, D, H, W]
            J (torch.Tensor): moving image with size [B, 1, D, H, W]
        ...
        if I_seg != None and J_seg is not None:
            Jw = self.warp(J_seg, flow_J, xyz)
            Iw = self.warp(I_seg, flow_I, xyz)
            if res != 1:
            ...
            if res != 1:
                Iw = F.interpolate(Iw, scale_factor=res, mode='nearest')
                Jw = F.interpolate(Jw, scale_factor=res, mode='nearest')

        if I_seg is not None and J_seg is not None or  self.loss_type == 'mse':
