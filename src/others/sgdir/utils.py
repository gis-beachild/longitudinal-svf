...
from data import dHCPRegistration
...
def get_dataset(config: dict, train: bool=True, **kwargs):
    ...
    """
    crop_x, crop_y, crop_z = config['csize']
    num_classes =  config['num_classes']
    dataset = dHCPRegistration(dataset_path=config['csv_path'], mode=train, crop_x=crop_x, crop_y=crop_y, crop_z=crop_z, num_classes=num_classes)
    return dataset
...
def animate_flow_3d(network: torch.nn.Module, fixed_img: torch.Tensor, moving_img: torch.Tensor, 
    ...
    mid_frame_index = fixed_img.size(4) // 2
    ...
    for index, t in enumerate(timesteps):
        ...
        ax[1].axis('off')
        xy = torch.cat([xyz[:, :, :, mid_frame_index, 1].unsqueeze(-1), xyz[:, :, :, mid_frame_index, 2].unsqueeze(-1)], dim=-1)
        ...
        ax[1].axis('off')
        xyr = torch.cat([xyzr[:, :, :, mid_frame_index, 1].unsqueeze(-1), xyzr[:, :, :, mid_frame_index, 2].unsqueeze(-1)], dim=-1)
