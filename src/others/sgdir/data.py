...
class dHCPRegistration(Dataset):
    def \
            __init__(self, dataset_path: str, mode: bool = True, crop_x=128, crop_y=128, crop_z=128,
                 num_classes=20) -> None:
        super().__init__()
        ...
        for index, row in df.iterrows():
            data_paths.append((row['image'], row['label']))
        self.crop = (crop_x, crop_y, crop_z)
        self.num_classes = num_classes
        self.combinaisons = []
        for i in range(1):
            self.combinaisons.append((data_paths[0], data_paths[-1]))

    def load_image_pair(self, fixed_id: str, moving_id: str) -> Tuple[
        np.ndarray]:
        """
        ...
        fixed_img = nib.load(fixed_id[0]).get_fdata()
        fixed_seg35 = nib.load(fixed_id[1]).get_fdata()

        # loading the moving image
        moving_img = nib.load(moving_id[0]).get_fdata()
        moving_seg35 = nib.load(moving_id[1]).get_fdata()
    ...
    def __len__(self):
        return len(self.combinaisons)

    def __getitem__(self, index):
        moving_id, fixed_id = self.combinaisons[index]
        moving = tio.ScalarImage(moving_id[0])
        moving_seg = tio.ScalarImage(moving_id[1])
        fixed = tio.ScalarImage(fixed_id[0])
        fixed_seg = tio.ScalarImage(fixed_id[1])

        transform = tio.Compose([
            tio.CropOrPad(target_shape=self.crop),
            tio.RescaleIntensity(out_min_max=(0, 1), percentiles=(0.5, 99.5)),
        ])

        transform_seg = tio.Compose([
            tio.CropOrPad(target_shape=self.crop),
        ])

        moving = transform(moving)[tio.DATA].float()
        moving_seg = transform_seg(moving_seg)[tio.DATA].float()
        fixed = transform(fixed)[tio.DATA].float()
        fixed_seg = transform_seg(fixed_seg)[tio.DATA].float()

        if self.mode == 'train':
            fixed = fixed + np.random.normal(scale=0.01, size=fixed.shape)
            moving = moving + np.random.normal(scale=0.01, size=moving.shape)
        self.d, self.h, self.w = fixed.shape[1:]
        xyz = self.get_grid()
...
class OASISRegistrationV2(Dataset):
    ...
    def load_image_pair(self, fixed_id: str, moving_id: str, crop_x: int, crop_y: int, crop_z: int) -> Tuple[
        np.ndarray]:
        """
        This method receives the ids of the fixed and moving images along with croping sizes along each axis
        and returns the center-cropped fixed and moving images with the segmentation masks.
        ...
        fixed_img, fixed_seg35, moving_img, moving_seg35 = self.__crop(
            [fixed_img, fixed_seg35, moving_img, moving_seg35], crop_x, crop_y, crop_z)
    ...
    def __getitem__(self, index):
        ...
        fixed, moving, fixed_seg, moving_seg = self.load_image_pair(fixed_id, moving_id, self.crop_x, self.crop_y,
                                                                    self.crop_z)
...
class LPBA40Registration(Dataset):
    ...
    def load_image_pair(self, fixed_id, moving_id, crop_x, crop_y, crop_z):
        ...
        # loading the fixed image
        fixed_img = \
        nib.load(os.path.join(image_path, f'{fixed_id}/{fixed_id}.delineation.skullstripped.img')).get_fdata()[..., 0]
        fixed_seg = \
        nib.load(os.path.join(image_path, f'{fixed_id}/{fixed_id}.delineation.structure.label.img')).get_fdata()[..., 0]

        # loading the moving image
        moving_img = \
        nib.load(os.path.join(image_path, f'{moving_id}/{moving_id}.delineation.skullstripped.img')).get_fdata()[..., 0]
        moving_seg = \
        nib.load(os.path.join(image_path, f'{moving_id}/{moving_id}.delineation.structure.label.img')).get_fdata()[
            ..., 0]
