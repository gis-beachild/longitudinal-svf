...
from src.others.sgdir.utils import SSIM3d
from src.others.sgdir.utils import get_dataset
from src.others.sgdir.utils import dice
from src.others.sgdir.utils import jacobian_determinant_3d
...
def main(args):
    ...
    name = config['name']
    savePath  = os.path.join(args.savePath, "sgdir", name)
    ...
    dataset = get_dataset(config=data_config, train=True)
    loader = DataLoader(dataset, shuffle=True, batch_size=batch_size)

    # loading the validation dataset
    val_dataset = get_dataset(config=data_config, train=False, val=True)
    ...
    for epoch in ss_iters:
        ...
        for sample in loader:
            ...
            for level in range(res_levels):
                I_, J_, xyz_, seg_I, seg_J = sample[0].to(device).float(), sample[1].to(device).float(), sample[2].to(
                    device).float(), sample[3].to(device).float(), sample[4].to(device).float()
                ss_optimizer.zero_grad()
                loss, ss_loss = network.loss_flow(I_, J_, seg_I, seg_J, xyz_, res[level])
            ...
            with torch.no_grad():
                seg_I = torch.argmax(seg_I, dim=1, keepdim=True).float()
                seg_J = torch.argmax(seg_J, dim=1, keepdim=True).float()

                xyz = network(I_, J_, xyz_, torch.ones(1, device=device))
                xyzr = network(I_, J_, xyz_, -torch.ones(1, device=device))
                tf_dice, _ = dice(seg_I.float(), seg_J.float(), xyz, xyzr, structured=True)
        ...
        for sample in val_loader:
            I_, J_, xyz_, seg_I, seg_J = sample[0].to(device), sample[1].to(device), sample[2].to(device), sample[3].to(
                device), sample[4].to(device)
            ...
            with torch.no_grad():
                ...
                warped_seg = torch.nn.functional.grid_sample(seg_J, xyz, padding_mode='reflection', align_corners=True)
        ...
        if epoch % ss_eval_interval == 0 or epoch == ss_epochs - 1:
            ...
            if avg_f_dice2 > best_dice:
                ...
                torch.save(network.state_dict(), savePath + './model.pth')
                log += ' [CHCKPNT]'
                print("best")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_yaml", type=str, help='Path to the dataset yaml file', required=False,
                        default="/home/florian/Documents/Programs/longitudinal-svf-pair/src/configs/data/atlasimages.yaml")
